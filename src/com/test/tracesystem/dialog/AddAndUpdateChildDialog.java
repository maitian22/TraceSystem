...
public class AddAndUpdateChildDialog extends Dialog implements
	...
	public AddAndUpdateChildDialog(Context context, TotalTableEntity entitys,
			String type, String[] data, String[] dataTitle, Handler handler) {
		super(context);
	...
	public void onClick(View v) {
		...
		case R.id.ensure_btn:
			...
			if (columName == null) {
				...
				if (type.equals("insert")) {
					result = DataBaseOperater.getInstance(context)
							.insertDataChild(entitys.getChildEnglish(),
									columName);
					showMessage = "添加成功";
				} else if (type.equals("updata")) {
					result = DataBaseOperater.getInstance(context)
							.updataDataChild(entitys.getChildEnglish(),
									columName, Long.parseLong(data[0]));
					showMessage = "修改成功";
				}
				if (result) {
